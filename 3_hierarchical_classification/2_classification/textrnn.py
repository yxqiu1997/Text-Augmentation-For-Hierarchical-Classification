...
# Build model
model = TextRNN(MAX_LENGTH, MAX_FEATURE, EMBEDDING_DIMENSIONS)
model.compile('adam', 'categorical_crossentropy', metrics=['accuracy'])

# Fit
early_stopping = EarlyStopping(monitor='val_accuracy', patience=30, mode='max')

ck_callback = ModelCheckpoint('./checkpoints/textrnn/weights.{epoch:02d}-{val_f1:.4f}.hdf5',
                              monitor='val_f1',
                              mode='max', verbose=2,
                              save_best_only=True,
                              save_weights_only=True)
tb_callback = TensorBoard(log_dir='./logs/textrnn')
...
model_name = 'textrnn-model-' + JOB_NAME + '.best'
model_dir = os.path.join(base_dir, 'models', 'textrnn', model_name)
