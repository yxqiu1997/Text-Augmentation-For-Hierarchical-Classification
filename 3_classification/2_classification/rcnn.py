...
train_padded_seqs_current = train_padded_seqs
train_padded_seqs_left = np.hstack([np.expand_dims(train_padded_seqs[:, 0], axis=1), train_padded_seqs[:, 0:-1]])
train_padded_seqs_right = np.hstack([train_padded_seqs[:, 1:], np.expand_dims(train_padded_seqs[:, -1], axis=1)])

test_padded_seqs_current = test_padded_seqs
test_padded_seqs_left = np.hstack([np.expand_dims(test_padded_seqs[:, 0], axis=1), test_padded_seqs[:, 0:-1]])
test_padded_seqs_right = np.hstack([test_padded_seqs[:, 1:], np.expand_dims(test_padded_seqs[:, -1], axis=1)])

# Build model
model = RCNN(MAX_LENGTH, MAX_FEATURE, EMBEDDING_DIMENSIONS)
model.compile('adam', 'categorical_crossentropy', metrics=['accuracy'])

# Fit
early_stopping = EarlyStopping(monitor='val_accuracy', patience=3, mode='max')

ck_callback = ModelCheckpoint('./checkpoints/rcnn/weights.{epoch:02d}-{val_f1:.4f}.hdf5',
                              monitor='val_f1',
                              mode='max', verbose=2,
                              save_best_only=True,
                              save_weights_only=True)
tb_callback = TensorBoard(log_dir='./logs/rcnn')

model.fit([train_padded_seqs_current,
           train_padded_seqs_left,
           train_padded_seqs_right], train_labels,
          batch_size=64, epochs=2,
          callbacks=[early_stopping,
                     Metrics(valid_data=([test_padded_seqs_current,
                                          test_padded_seqs_left,
                                          test_padded_seqs_right], test_labels)),
                     ck_callback, tb_callback],
          validation_data=([test_padded_seqs_current,
                            test_padded_seqs_left,
                            test_padded_seqs_right], test_labels),
          verbose=0)

model_name = 'rcnn_script-model-' + JOB_NAME + '.best'
model_dir = os.path.join(base_dir, 'models', 'rcnn_script', model_name)
