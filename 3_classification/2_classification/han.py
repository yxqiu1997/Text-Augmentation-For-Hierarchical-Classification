...
'''
    Command line usage:
        ...
        Using training set augmented by deletion/insertion/swap/synonyms:
            python han.py deletion/insertion/swap/synonyms
...
train_padded_seqs = pad_sequences(train_word_ids,
                                  maxlen=MAX_SENTENCE_LENGTH * MAX_WORD_LENGTH)
test_padded_seqs = pad_sequences(test_word_ids,
                                 maxlen=MAX_SENTENCE_LENGTH * MAX_WORD_LENGTH)
train_padded_seqs = train_padded_seqs.reshape((len(train_padded_seqs),
                                               MAX_SENTENCE_LENGTH,
                                               MAX_WORD_LENGTH))
test_padded_seqs = test_padded_seqs.reshape((len(test_padded_seqs),
                                             MAX_SENTENCE_LENGTH,
                                             MAX_WORD_LENGTH))

# Build model
model = HAN(MAX_SENTENCE_LENGTH, MAX_WORD_LENGTH, MAX_FEATURE, EMBEDDING_DIMENSIONS)
model.compile('adam', 'categorical_crossentropy', metrics=['accuracy'])

# Fit
early_stopping = EarlyStopping(monitor='val_accuracy', patience=10, mode='max')

ck_callback = ModelCheckpoint('./checkpoints/han/weights.{epoch:02d}-{val_f1:.4f}.hdf5',
                              monitor='val_f1',
                              mode='max', verbose=2,
                              save_best_only=True,
                              save_weights_only=True)
tb_callback = TensorBoard(log_dir='./logs/han')
...
model_name = 'han-model-' + JOB_NAME + '.best'
model_dir = os.path.join(base_dir, 'models', 'han', model_name)
